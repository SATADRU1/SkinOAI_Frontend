...
export default function ScanScreen() {
  ...
  const handleCapture = async () => {
    ...
    try {
      const photo = await (cameraRef.current as any).takePictureAsync({ base64: true, quality: 0.8 });
      if (!photo || !photo.base64) {
        setIsAnalyzing(false);
        Alert.alert("Capture Error", "No image data captured. Please try again.");
        return;
      }
      setIsAnalyzing(false);
      router.push({
      ...
      setIsAnalyzing(false);
      Alert.alert("Capture Error", `Failed to capture image. ${(error as any)?.message || "Please try again."}`);
    }
  };
  ...
  const handleUploadPhoto = async () => {
    try {
      ...
      if (result.canceled) {
        Alert.alert("Upload Cancelled", "Image selection was cancelled.");
        return;
      }
